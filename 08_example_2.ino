...
#define INTERVAL 25      // sampling interval (unit: msec) 샘플링 주기를 25ms로 변경
...
void loop() {
  ...
  int brightness = calLEDBrightness(distance);
...
// 거리에 비례하여 LED의 밝기를 제어하는 함수 추가
int calLEDBrightness(float distance) {
  if (distance < _DIST_MIN || distance > _DIST_MAX) {
    return 255;
    }
  if (distance <= 200.0) {
    return map(distance, _DIST_MIN, 200,250,0);
    }
  else {
    return map(distance, 200, _DIST_MAX, 0, 255);
    }
  }
